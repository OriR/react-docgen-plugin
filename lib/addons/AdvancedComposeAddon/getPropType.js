...
function isNotOrdinaryComposition(path) {
  return (
    types.MemberExpression.check(path.node) && !printValue(path.get('object')).includes('PropTypes')
  );
}
...
function getPropTypeArrayOf(argumentPath, allCompositions) {
  ...
  if (subType.name === 'unknown') {
    type.value = { name: 'composes', raw: printValue(argumentPath) };
    type.computed = true;
...
function getPropTypeShape(argumentPath, allCompositions) {
  ...
  if (types.ObjectExpression.check(argumentPath.node)) {
    ...
    argumentPath.get('properties').each(function(propertyPath) {
      if (
        types.SpreadProperty.check(propertyPath.node) ||
        types.SpreadElement.check(propertyPath.node)
      ) {
...
getPropType = (path, allCompositions = {}) => {
  ...
  if (!descriptor) {
    ...
    if (types.Identifier.check(node) && simplePropTypes.hasOwnProperty(node.name)) {
      ...
      descriptor.value = descriptor.required ? printValue(path.get('object')) : printValue(path);
      ...
      visit(root, {
        visitVariableDeclarator: node => {
          if (printValue(node.get('id')) === propMember) {
            if (!types.ArrowFunctionExpression.check(node.get('init').value)) {
