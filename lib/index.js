...
function ReactDocGenPlugin(options) {
  this.defaultIsComposingComponent = ({ composedComponentDisplayName, composedFile }) => composedFile.match(new RegExp(`[\\\\|\\/](${composedComponentDisplayName})\\.`));
  ...
  compiler.plugin('emit', function (compilation, done) {
    ...
    const getComposes = (componentDefinition, file) => {
      ...
      return composes.reduce((allComposed, composedModule) => {
        ...
        const composingComponentDefinitions = loaded[composedFile].filter(composedFileComponentDefinition => self.options.isComposingComponent({ composingFile: file, composedFile, composingFileDisplayName: componentDefinition.displayName,composedComponentDisplayName: composedFileComponentDefinition.displayName }, self.defaultIsComposingComponent));
